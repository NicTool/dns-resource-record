...
export function to(rrInstance) {
  // tinydns data files have no class field; anything but IN would be silently corrupted
  if (rrInstance.get('class') !== 'IN')
    throw new Error(
      `${rrInstance.get('type')}: tinydns supports only class IN, got ${rrInstance.get('class')}`,
    )
