...
/**
 * RFC 3597 §5: a record decoded from generic (opaque) rdata must re-encode to
 * the same bytes, else the rdata was misparsed (extra, missing, or malformed
 * bytes). ASCII case may differ: embedded names are normalized to lowercase.
 */
export function assertWireRoundTrip(rrInstance, rdata) {
  ...
  if (!same) throw new Error(`${rrInstance.get('type')}: rdata does not round-trip through wire encoding`)
}
