import { hexToBytes } from './binary.js'
...
export function fromBind3597(rrInstance, parsed) {
  ...
  const generic = /^TYPE(\d+)$/i.exec(parsed.type)
  const typeId = generic ? parseInt(generic[1], 10) : undefined
  const staticId = rrInstance.constructor.typeId
  if (typeId !== undefined && staticId !== undefined && typeId !== staticId)
    throw new Error(`${rrInstance.constructor.typeName}: ${parsed.type} does not match type id ${staticId}`)
  if (typeId === undefined && staticId === undefined)
    throw new Error(`UNKNOWN: generic rdata requires a TYPEnnn type (RFC 3597), got: ${parsed.type}`)
