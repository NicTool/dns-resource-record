...
export function escapeBindName(name) {
  ...
  return Array.from(name, (char) => {
    if (!/[\s;"\\()]/u.test(char) && char.codePointAt(0) >= 32 && char.codePointAt(0) !== 127) return char
    return Array.from(encoder.encode(char), (byte) => '\\' + String(byte).padStart(3, '0')).join('')
  }).join('')
}

function characterString(token) {
  const text = token.startsWith('"') ? token.slice(1, -1) : token
  const bytes = []
  const encoder = new TextEncoder()
